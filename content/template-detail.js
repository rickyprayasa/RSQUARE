    ...
    try {
        ...
        if (product) {
            ...
            const tombolBeliManualHTML = `
                <a href="${linkBeliManual}" class="btn-primary inline-block flex items-center justify-center w-full px-8 py-3 rounded-lg font-semibold">
            ...
            const externalButtonsHTML = product.detail.link_pembelian.map(link => `
                <a href="${link.url}" target="_blank" rel="noopener noreferrer" class="btn-btn-primary inline-block flex items-center justify-center w-full px-8 py-3 rounded-lg font-semibold">
