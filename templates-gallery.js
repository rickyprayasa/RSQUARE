document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        if (!indexResponse.ok) throw new Error(`Gagal memuat daftar produk (_index.json). Status: ${indexResponse.status}`);
        ...
        const allCardsHTML = products.map(product => {
            ...
            // Kita pastikan path gambar tidak diawali dengan '/'
            let imagePath = product.gambar_thumbnail || ''; // Ambil path, atau string kosong jika tidak ada
            if (imagePath.startsWith('/')) {
                // Jika path diawali '/', hapus karakter pertama (yaitu '/')
        ...
        galleryContainer.innerHTML = `<p class="text-center text-red-500 col-span-full">Maaf, terjadi kesalahan saat memuat koleksi template. Error: ${error.message}</p>`;
    }
});
